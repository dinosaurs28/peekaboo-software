...
export default function MovementReportPage() {
  ...
  const [from, setFrom] = useState<string>(() => isoDate(new Date(new Date().setDate(new Date().getDate() - 7))));
  const [to, setTo] = useState<string>(() => isoDate(new Date()));
  const [rows, setRows] = useState<MovementRow[]>([]);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    listCategories().then(c => setCategories(c.filter(x => x.active))).catch(() => undefined);
  }, []);
  ...
  async function run() {
    ...
    try {
      ...
      const data = await aggregateInventoryMovement(fromIso, toIso, { category: category || undefined });
      setRows(data);
    } finally {
      setBusy(false);
    }
  ...
  const totalOut = useMemo(() => rows.reduce((s, r) => s + r.qtyOut, 0), [rows]);
  ...
  function downloadCsv() {
    ...
    for (const r of rows) {
      const cols = [r.name, r.sku, r.category || '', String(r.qtyIn), String(r.qtyOut), String(r.net)];
        ...
        <main className="flex-1 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            <div>
              ...
              <input type="date" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={from} onChange={e => setFrom(e.target.value)} />
            </div>
            <div>
              <label className="text-sm text-muted-foreground">To</label>
              <input type="date" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={to} onChange={e => setTo(e.target.value)} />
            </div>
            <div>
            ...
            <div className="flex items-end">
              <button className="px-3 py-2 rounded-md border bg-background w-full" onClick={run} disabled={busy}>Apply</button>
            </div>
            <div className="flex items-end">
          ...
          <div className="border rounded-md overflow-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {rows.length === 0 && (
                  <tr><td className="px-3 py-6 text-center text-muted-foreground" colSpan={6}>No movement in range</td></tr>
                )}
              </tbody>
              {rows.length > 0 && (
                <tfoot>
