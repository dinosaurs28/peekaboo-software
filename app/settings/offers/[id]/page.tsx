...
export default function EditOfferPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setRuleType((o.ruleType as any) || (o.discountType === 'amount' ? 'flat' : o.discountType === 'percentage' ? 'percentage' : 'flat'));
      ...
      setSelectedCategories(Array.isArray(o.categoryNames) ? o.categoryNames : []);
    }
    load();
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/settings/offers" className="px-3 py-2 rounded-md border bg-background text-sm">Back</Link>
      </div>
      <form className="space-y-4" onSubmit={onSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Active</label>
            <select className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={String(active)} onChange={(e) => setActive(e.target.value === 'true')}>
          ...
          <div>
            ...
            <input type="datetime-local" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} />
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Ends At</label>
            <input type="datetime-local" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={endsAt} onChange={(e) => setEndsAt(e.target.value)} />
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Discount Type</label>
            <select className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={discountType} onChange={(e) => setDiscountType(e.target.value as any)}>
          ...
          <div>
            ...
            <input type="number" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={discountValue} onChange={(e) => setDiscountValue(Number(e.target.value || 0))} />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-muted-foreground">Rule Type</label>
            <select className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={ruleType} onChange={(e) => setRuleType(e.target.value as any)}>
          ...
          {ruleType === 'bogoSameItem' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={buyQty} onChange={(e) => setBuyQty(Number(e.target.value || 0))} />
              </div>
              <div>
                <label className="text-sm text-muted-foreground">Get Qty</label>
                <input type="number" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={getQty} onChange={(e) => setGetQty(Number(e.target.value || 0))} />
              </div>
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={String(dobMonthOnly)} onChange={(e) => setDobMonthOnly(e.target.value === 'true')}>
                <option value="false">No</option>
                <option value="true">Yes</option>
              </select>
            </div>
            <div>
              ...
              <input className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={eventName} onChange={(e) => setEventName(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-muted-foreground">Priority</label>
              <input type="number" className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={priority} onChange={(e) => setPriority(Number(e.target.value || 0))} />
            </div>
            <div>
              <label className="text-sm text-muted-foreground">Exclusive</label>
              <select className="w-full h-9 rounded-md border bg-background px-2 text-sm" value={String(exclusive)} onChange={(e) => setExclusive(e.target.value === 'true')}>
                <option value="false">No</option>
                <option value="true">Yes</option>
              </select>
            </div>
        ...
        <div className="flex gap-2">
          <button type="submit" className="px-3 py-2 rounded-md border bg-emerald-600 text-white" disabled={saving}>{saving ? 'Saving…' : 'Save Changes'}</button>
          <Link href="/settings/offers" className="px-3 py-2 rounded-md border bg-background">Cancel</Link>
        </div>
      </form>
    </div>
  );
