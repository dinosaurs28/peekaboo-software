...
function toLog(id: string, data: Record<string, unknown>): InventoryLogDoc {
  ...
  const t =
    ...
    ["adjustment", "sale", "purchase", "return", "damage"].includes(data.type)
      ? (data.type as InventoryLogDoc["type"])
      : "adjustment";
  return {
    id,
    ...
    quantityChange:
      typeof data.quantityChange === "number"
        ? data.quantityChange
        : Number(data.quantityChange || 0),
    reason: typeof data.reason === "string" ? data.reason : undefined,
    relatedInvoiceId:
      typeof data.relatedInvoiceId === "string"
...
export default function InventoryLogsPage() {
  ...
  useEffect(() => {
    ...
    if (from)
      constraints.push(where("createdAt", ">=", new Date(from).toISOString()));
    if (to) {
    ...
    const unsub = onSnapshot(q, (snap: QuerySnapshot<DocumentData>) => {
      setLogs(
        snap.docs.map((d) => toLog(d.id, d.data() as Record<string, unknown>))
      );
    });
    return () => unsub();
  }, [from, to, prodId, type]);
