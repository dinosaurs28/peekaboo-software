...
export default function DashboardPage() {
  ...
  const { fromIso, toIso, rangeDays } = useMemo(() => {
    if (!fromDate || !toDate)
      return { fromIso: undefined, toIso: undefined, rangeDays: 0 };
    ...
    const unsub = onSnapshot(
      ...
      (snap: QuerySnapshot<DocumentData>) => {
        ...
        snap.docs.forEach((d) => {
          ...
          const issuedAt =
            typeof data.issuedAt === "string" ? data.issuedAt : undefined;
          const dayKey = issuedAt?.slice(0, 10);
          if (dayKey)
            dayMap.set(dayKey, (dayMap.get(dayKey) || 0) + validGrand);
          ...
          items.forEach((it) => {
            ...
            const lineRevenue = Math.max(
              0,
              unitPrice * validQty - (Number.isFinite(disc) ? disc : 0)
            );

            const name =
              typeof it.name === "string" && it.name
                ? it.name
                : productMeta[pid]?.name || pid;
            ...
            if (productMeta[pid]?.category)
              cur.category = productMeta[pid].category;
            agg.set(pid, cur);
          });
        });
  ...
  useEffect(() => {
    ...
    const q = query(
      col,
      where("issuedAt", ">=", prevStart),
      where("issuedAt", "<=", prevEnd)
    );
    ...
    getDocs(q)
      .then((snap) => {
        ...
        snap.docs.forEach((d) => {
          const grand =
            typeof d.data().grandTotal === "number"
              ? d.data().grandTotal
              : Number(d.data().grandTotal ?? 0);
                  ...
                  <div className="space-y-6">
                      ...
                      {(() => {
                        const pct =
                          prevRevenue > 0
                            ...
                            ? 100
                            : 0;
                        const pctStr = `${pct >= 0 ? "+" : ""}${pct.toFixed(
                          1
                        )}%`;
                        const label =
                          rangeDays > 1
                        ...
                        const cls =
                          pct > 0
                            ...
                            : "text-gray-500";
                        return (
                          <p className={`text-sm font-medium mt-2 ${cls}`}>
                            {label} {pctStr}
                          </p>
                        );
                      })()}
                    ...
                    <div className="h-56 flex items-end gap-1 border-b border-gray-200 pb-4 overflow-x-auto">
                      {(() => {
                        const max = Math.max(
                          1,
                          ...revenueBuckets.map((b) => b.total)
                        );
                        return revenueBuckets.length > 0 ? (
                          revenueBuckets.map((b) => {
                            const h = Math.max(
                              8,
                              Math.round((b.total / max) * 200)
                            );
                            return (
                              <div
                                key={b.label}
                    ...
                    {revenueBuckets.length > 0 && (
                      <div className="flex justify-between text-xs text-gray-500">
                        ...
                        <span>
                          {revenueBuckets[revenueBuckets.length - 1]?.label}
                        </span>
                      </div>
                    )}
                  </div>
