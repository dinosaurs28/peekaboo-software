...
function assertDb() { if (!db) throw new Error("Firestore not initialized"); }
...
export async function createCustomer(input: UpsertCustomerInput): Promise<string> {
  assertDb();
  const col = collection(db!, COLLECTIONS.customers);
...
export async function updateCustomer(id: string, input: Partial<UpsertCustomerInput>): Promise<void> {
  assertDb();
